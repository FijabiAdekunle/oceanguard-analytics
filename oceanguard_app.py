...
with st.sidebar:
    ...
    st.markdown("[Portfolio](https://sites.google.com/view/fijabi-j-adekunle/uk-maib-maritime-accidents-2020-2024)")
...
with c4:
    st.markdown(f"""<div class="kpi-card kpi-card-blue">
        <div class="kpi-value" style="font-size:1rem">{top_vessel}</div>
        <div class="kpi-label">Top Vessel Type</div>
    </div>""", unsafe_allow_html=True)

with c5:
    short_injury = top_injury[:22] + "…" if len(top_injury) > 22 else top_injury
    st.markdown(f"""<div class="kpi-card kpi-card-gold">
        <div class="kpi-value" style="font-size:0.95rem">{short_injury}</div>
        <div class="kpi-label">Top Injury Type</div>
    </div>""", unsafe_allow_html=True)

with c6:
    short_loc = top_location[:22] + "…" if len(top_location) > 22 else top_location
    st.markdown(f"""<div class="kpi-card">
        <div class="kpi-value" style="font-size:0.95rem">{short_loc}</div>
